...
echo "<h1><b> Прайс - лист на аудио: </b></h1>". "<p>";
echo '</div>';
...
function TabletsCompare ($amount1,$amount2){
	...
	echo '</div>';
	echo "<h2><b> Общие характеристики: </b></h1>". "<p>". 'Принтер: '  .htmlspecialchars ($item['name']). 'id- '. htmlspecialchars ($item['id']). "<p>". 'Описание:' .htmlspecialchars ($item['description']). "<p>" .' Идентификатор категории: ' .htmlspecialchars( $item['categoryId']). "<p>". 'цена: '.
	number_format($item['prices']['amount'],2,',',' '). " ".htmlspecialchars( $item['prices']['curCode']).
	"<p>". 'Поставщик: '.htmlspecialchars ($item['vendor']).' код поставщика: '.htmlspecialchars($item['vendorId'] ).
	"<p>" .'Количество: '.htmlspecialchars( $item['gradeCount'])."<p> Ссылка на yandex market:" .'<a href='.htmlspecialchars($item['link'])."'>";
	echo $item['link'];
	echo "</a>";
	echo '</div>' ; ?>
				...
				?>">
				<?php
				echo 'rating-'. htmlspecialchars ($item['rating']);
